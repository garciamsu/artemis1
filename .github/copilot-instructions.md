...
- En la ronda actual de pruebas solo están disponibles físicamente `HC-05`, `FC-51 derecho`, `FC-51 izquierdo`, `L298N motor derecho` y `L298N motor izquierdo`.
...
## Registro De Módulos
- `Bluetooth HC-05`: propósito actual validar comunicación serial bidireccional y servir como canal de comandos de prueba para el robot 2WD. Entradas: texto recibido por Bluetooth y por monitor serial USB. Salidas: respuestas por Bluetooth y trazas por serial USB. Dependencias de hardware: HC-05 en modo esclavo. Pinout usado en la prueba: `TXD HC-05 -> pin 2 Arduino`, `pin 3 Arduino -> divisor resistivo 2k/1k -> RXD HC-05`, `VCC -> 5V`, `GND -> GND`, `STATE` y `KEY` desconectados. Estado: validado en enlace básico. Hallazgos relevantes: módulo nuevo de fábrica, PIN de emparejamiento validado con `1234`, comunicación de texto confirmada con Android usando `Serial Bluetooth Terminal`, velocidad de trabajo actual `9600`; el firmware mantiene compatibilidad con comandos largos por texto y ahora soporta dos perfiles rápidos seleccionables: `simple` con `U`, `N`, `L`, `R`, `X`, `D`, `E`, `C`, `I`, `J`, `K`, `1`, `2`, `3`, y `arduino` para Arduino Bluetooth Controller con `F`, `B`, `L`, `R`, `G`, `H`, `I`, `J`, `S`, `1`, `2`, `3`, `4`; el cambio se hace con `perfil simple` o `perfil arduino`. Procedimiento mínimo: revisar cableado y divisor resistivo, abrir monitor serial a `9600`, enlazar Android, usar `estado` para confirmar el perfil activo, enviar `hola`, `ayuda`, `ping` y `estado`, y para apps con botones probar el mapa correspondiente al perfil seleccionado verificando eco y respuesta física.
...
- `HC-SR04 frontal`: propósito actual medir distancia al frente del robot. Entradas: `TRIG=7`, `ECHO=8`. Salidas: distancia en cm o lectura inválida si no hay eco. Dependencias de hardware: `5V`, `GND` común. Pinout usado en la prueba: sensor montado fijo mirando hacia adelante. Estado: pendiente por hardware no disponible en la ronda actual.
- `DHT22`: propósito actual medir temperatura y humedad ambiente. Entradas: `DATA=A0`. Salidas: temperatura y humedad. Dependencias de hardware: módulo con pull-up integrada, `5V`, `GND` común. Pinout usado en la prueba: `DATA` al pin analógico `A0`. Estado: pendiente por hardware no disponible en la ronda actual.
- `Adafruit TSL2591`: propósito actual medir luz ambiente por I2C. Entradas: `SDA=SDA`, `SCL=SCL`. Salidas: lectura de luminosidad y canales internos según librería. Dependencias de hardware: `5V`, `GND` común, `3Vo` e `INT` desconectados. Pinout usado en la prueba: conectado directamente al bus I2C del Arduino Uno sin otros módulos I2C activos por ahora. Estado: pendiente por hardware no disponible en la ronda actual.
...
- `HC-05` modo app gráfica: preparado en firmware. Resultado actual: se agregó soporte específico para Arduino Bluetooth Controller sin perder Serial Bluetooth Terminal; el perfil rápido activo puede fijarse dentro del programa o cambiarse por comando (`perfil arduino` o `perfil simple`); en el perfil Arduino Bluetooth Controller se implementaron `F/B/L/R/G/H/I/J/S/1/2/3/4` adaptados al robot 2WD con L298N y se dejaron `U/u` y `Y` como no disponibles por falta de LED y buzzer definidos en esta ronda. Para mejorar estabilidad en pruebas con apps gráficas, el firmware ahora suprime respuestas Bluetooth durante comandos rápidos repetidos y aplica auto-stop por inactividad Bluetooth tras una ventana corta de seguridad.
- `Firmware de diagnóstico modular`: realizado. Compilación verificada en PlatformIO y ajustada a los módulos disponibles actualmente: `HC-05`, `FC-51 derecho`, `FC-51 izquierdo`, `L298N motor derecho` y `L298N motor izquierdo`.
...
- `HC-SR04 frontal`: pendiente por módulo no disponible en esta ronda.
- `DHT22`: pendiente por módulo no disponible en esta ronda.
- `Adafruit TSL2591`: pendiente por módulo no disponible en esta ronda.